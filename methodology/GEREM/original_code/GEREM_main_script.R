...
# GEREM_main_script.R
#
# Historical implementation of the Glass Eel Recruitment Estimation Model
# (GEREM) used during Master's thesis research (2016).
#
# This file has been preserved in its original form for archival purposes.
#
# Documentation:
# - README.md
...
#
# The code below has not been modified from the archived implementation.
################################################################################
...
data_1_wide=reshape(data_1[,1:3],direction="wide",v.names="dat_value",timevar="loc_name",idvar="dat_year")
...
french_serie[which(french_serie$dat_year==2006),"Oria"]=1228
french_serie[which(french_serie$dat_year==2007),"Oria"]=682
french_serie[which(french_serie$dat_year==2008),"Oria"]=1044
#french_serie[which(french_serie$dat_year==2009),"Oria"]
french_serie[which(french_serie$dat_year==2010),"Oria"]=1001
french_serie[which(french_serie$dat_year==2011),"Oria"]=895
french_serie[which(french_serie$dat_year==2012),"Oria"]=1425
french_serie[which(french_serie$dat_year==2013),"Oria"]=1495
...
tryCatch({jags_res=run.jags("versionBugs2_1.txt",monitor=c("beta","tauIA","tauU","tauIP","tauIE","logq","loga","logp","logRglobal","Rbv","Rzone","propR"),data=mydata,n.chains=3,inits=generate_init,burnin=40000,sample=40000,method="parallel",thin=1,tempdir=FALSE,
         summarise=FALSE,adapt = 50000,keep.jags.files=TRUE)},err=function(err) {print(err); failed.jags()})
